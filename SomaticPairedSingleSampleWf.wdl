...
workflow SomaticPairedEndSingleSampleWorkflow {
  ...
  scatter (unmapped_bam in flowcell_unmapped_bams) {
    ...
    String sub_strip_path = "/cluster_share/.*/"
  ...
  # Replacing Picard SortSam with Samtools Sort for sorting
...
task SamtoolsSort {
  ...
  command <<<
    set -o pipefail
    set -e
    samtools sort -T $TMPDIR/${output_bam_basename}.bam.tmp -m 64M --threads 16 -l ${compression_level} ${input_bam} -o ${output_bam_basename}.bam && \
